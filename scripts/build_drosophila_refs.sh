#!/usr/bin/env bash
# Download and build Drosophila melanogaster (BDGP6.46) reference indices for eFISHent.
#
# Downloads genome, GTF, builds bowtie2 + jellyfish indices, generates
# transcriptome + BLAST DB. Output goes to ./refs_drosophila/
#
# Requirements: bowtie2, jellyfish, gffread, makeblastdb, samtools (for faidx)
# Estimated time: ~20-40 minutes (~140 MB genome)
# Estimated disk: ~1.5 GB
#
# Usage:
#   bash scripts/build_drosophila_refs.sh [output_dir]

set -euo pipefail

OUT="${1:-refs_drosophila}"
mkdir -p "$OUT"
cd "$OUT"
...
echo "=== Building Drosophila (${ASSEMBLY}) reference indices ==="
...
if [ -f "${SPECIES}.${ASSEMBLY}.dna.toplevel_15.jf" ]; then
    ...
    jellyfish count -m 15 -s 500M -t 4 -C \
        "${SPECIES}.${ASSEMBLY}.dna.toplevel.fa" \
...
if [ -f "transcriptome.fa" ]; then
    ...
    if ! command -v gffread >/dev/null 2>&1; then
        ...
        echo "  brew install gffread  OR  build from https://github.com/gpertea/gffread"
        exit 1
    fi
...
echo ""
echo "=== All Drosophila indices built ==="
...
echo ""
echo "Next steps:"
echo "  1. Run: bash scripts/prepare_drosophila_hf_upload.sh $(pwd) /tmp/efishent-hf"
echo "  2. Push: cd /tmp/efishent-hf && git add . && git commit -m 'Add BDGP6 Drosophila indices' && git push"
