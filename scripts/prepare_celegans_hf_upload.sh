#!/usr/bin/env bash
# Prepare C. elegans (WBcel235) refs for HuggingFace upload.
# Creates the expected directory structure with standardized filenames.
#
# Usage:
#   bash scripts/prepare_celegans_hf_upload.sh ./refs_celegans /tmp/efishent-hf
...
GENOME_DIR="$HF_DIR/caenorhabditis_elegans/WBcel235"
mkdir -p "$GENOME_DIR"

echo "Copying and renaming files from $REFS_DIR to $GENOME_DIR..."

# Genome FASTA + index
cp "$REFS_DIR/Caenorhabditis_elegans.WBcel235.dna.toplevel.fa" "$GENOME_DIR/genome.fa"
cp "$REFS_DIR/Caenorhabditis_elegans.WBcel235.dna.toplevel.fa.fai" "$GENOME_DIR/genome.fa.fai"

# Bowtie2 indices
for i in 1 2 3 4; do
    cp "$REFS_DIR/Caenorhabditis_elegans.WBcel235.dna.toplevel.$i.bt2" "$GENOME_DIR/genome.$i.bt2"
done
for i in 1 2; do
    cp "$REFS_DIR/Caenorhabditis_elegans.WBcel235.dna.toplevel.rev.$i.bt2" "$GENOME_DIR/genome.rev.$i.bt2"
done

# GTF annotation
cp "$REFS_DIR/Caenorhabditis_elegans.WBcel235.115.gtf" "$GENOME_DIR/annotation.gtf"

# Jellyfish index (k=15)
cp "$REFS_DIR/Caenorhabditis_elegans.WBcel235.dna.toplevel_15.jf" "$GENOME_DIR/genome_15.jf"
...
echo "  cd $HF_DIR"
echo "  git add ."
echo "  git commit -m 'Add WBcel235 C. elegans genome indices'"
echo "  git push"
