#!/usr/bin/env bash
# Download and build C. elegans (WBcel235) reference indices for eFISHent.
#
# Downloads genome, GTF, builds bowtie2 + jellyfish indices, generates
# transcriptome + BLAST DB. Output goes to ./refs_celegans/
#
# Requirements: bowtie2, jellyfish, gffread, makeblastdb, samtools (for faidx)
# Estimated time: ~15-30 minutes (~100 MB genome)
# Estimated disk: ~1 GB
#
# Usage:
#   bash scripts/build_celegans_refs.sh [output_dir]
...
echo "=== Building C. elegans (${ASSEMBLY}) reference indices ==="
...
echo ""
echo "=== All C. elegans indices built ==="
...
echo ""
echo "Next steps:"
echo "  1. Run: bash scripts/prepare_celegans_hf_upload.sh $(pwd) /tmp/efishent-hf"
echo "  2. Push: cd /tmp/efishent-hf && git add . && git commit -m 'Add WBcel235 C. elegans indices' && git push"
