#!/usr/bin/env bash
# Download and build Zebrafish (GRCz11) reference indices for eFISHent.
#
# Requirements: bowtie2, jellyfish, gffread, makeblastdb, samtools
# Estimated time: ~1-2 hours (~1.4 GB genome)
# Estimated disk: ~10 GB
#
# Usage:
#   bash scripts/build_zebrafish_refs.sh [output_dir]

set -euo pipefail

OUT="${1:-refs_zebrafish}"
mkdir -p "$OUT"
cd "$OUT"
...
echo "=== Building Zebrafish (${ASSEMBLY}) reference indices ==="
...
echo ""
echo "=== All Zebrafish indices built ==="
