#!/usr/bin/env bash
# Download and build yeast (S. cerevisiae R64-1-1) reference indices for eFISHent.
#
# Downloads genome, GTF, builds bowtie2 + jellyfish indices, generates
# transcriptome + BLAST DB. Output goes to ./refs_yeast/
#
# Requirements: bowtie2, jellyfish, gffread, makeblastdb, samtools (for faidx)
# Estimated time: ~5 minutes (~12 MB genome)
# Estimated disk: ~200 MB
#
# Usage:
#   bash scripts/build_yeast_refs.sh [output_dir]
...
echo "=== Building yeast (${ASSEMBLY}) reference indices ==="
...
if [ -f "${SPECIES}.${ASSEMBLY}.dna.toplevel_15.jf" ]; then
    ...
    jellyfish count -m 15 -s 100M -t 4 -C \
        "${SPECIES}.${ASSEMBLY}.dna.toplevel.fa" \
...
echo "  2. Push: cd /tmp/efishent-hf && git add . && git commit -m 'Add R64 yeast indices' && git push"
