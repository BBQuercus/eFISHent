#!/usr/bin/env bash
# Prepare Rat (GRCr8) refs for HuggingFace upload.
#
# Usage:
#   bash scripts/prepare_rat_hf_upload.sh ./refs_rat /tmp/efishent-hf
...
GENOME_DIR="$HF_DIR/rattus_norvegicus/GRCr8"
mkdir -p "$GENOME_DIR"

echo "Copying and renaming files from $REFS_DIR to $GENOME_DIR..."

# Genome FASTA + index
cp "$REFS_DIR/Rattus_norvegicus.GRCr8.dna.toplevel.fa" "$GENOME_DIR/genome.fa"
cp "$REFS_DIR/Rattus_norvegicus.GRCr8.dna.toplevel.fa.fai" "$GENOME_DIR/genome.fa.fai"

# Bowtie2 indices
for i in 1 2 3 4; do
    cp "$REFS_DIR/Rattus_norvegicus.GRCr8.dna.toplevel.$i.bt2" "$GENOME_DIR/genome.$i.bt2"
done
for i in 1 2; do
    cp "$REFS_DIR/Rattus_norvegicus.GRCr8.dna.toplevel.rev.$i.bt2" "$GENOME_DIR/genome.rev.$i.bt2"
done

# GTF annotation
cp "$REFS_DIR/Rattus_norvegicus.GRCr8.115.gtf" "$GENOME_DIR/annotation.gtf"

# Jellyfish index (k=15)
cp "$REFS_DIR/Rattus_norvegicus.GRCr8.dna.toplevel_15.jf" "$GENOME_DIR/genome_15.jf"
...
metadata = {
    'genome': 'Rattus norvegicus (GRCr8)',
